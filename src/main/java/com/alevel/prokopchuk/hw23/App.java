...
import com.alevel.prokopchuk.hw23.beans.EmployeeBean;
import com.alevel.prokopchuk.hw23.entities.Department;
import com.alevel.prokopchuk.hw23.entities.Employee;
import com.alevel.prokopchuk.hw23.entities.Gender;
...
//    private static final String password = "";
    ...
    private static void part2() {
        ...
        try (Connection connection = ConnectorDB.getConnection();
            ...
            while (rs.next()) {
                ...
                employee = new EmployeeBean(id, firstName, lastName, title, totalSalary, hireDate);
                //counter++;
            ...
            for (EmployeeBean employeePointer : employees) {
                System.out.println(employeePointer);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private static void part4(int threshold, int increase) {
        ...
        final String sqlUpdate = "update employees.salaries set salaries.salary = ? " +
                "where salaries.to_date > now() and salaries.emp_no = ?;";
        try (Connection connection = ConnectorDB.getConnection()) {
            ...
            while (rs.next()) {
                ...
                int totalSalary = rs.getInt(6);
                employee = new EmployeeBean(id, firstName, lastName, title, currentSalary, totalSalary);
                employees.add(employee);
            }
            st = connection.prepareStatement(sqlUpdate);
            for (EmployeeBean employeePointer : employees) {
                System.out.println(employeePointer);
                st.setInt(1, employeePointer.getCurrentSalary() + increase);
                st.setInt(2, employeePointer.getId());
                st.executeUpdate();
            }
            System.out.println("Found " + employees.size() + " employees");
